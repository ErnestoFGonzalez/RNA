...
def hydrosheds():
    ...
    # Download GloRic shapefile at https://www.hydrosheds.org/page/gloric
...
def merge_csvs():
    ...
    if not os.path.exists('data_pmm/' + csvs[0][45:69] + '-' + csvs[-1][45:69] + '.csv'):
        import pandas as pd
        data = pd.read_csv(csvs[0], delimiter=',')
        ...
        data.join(pd.read_pickle('data_gloric/areas_gloric_no_geo.pkl').set_index('Reach_ID').area_sk)\
            .reset_index()\
